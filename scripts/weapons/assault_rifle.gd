...
# --- WEAPON RESOURCES ---
const BULLET = preload("res://scenes/weapons/bullets/rifle_bullet.tscn")
...
func reload():
	if current_ammo < magazine_size and reserve_ammo > 0:
		...
		if current_ammo == 0:
			reload_timer.wait_time = 2.5 # Slower empty reload for rifle
			reload_empty_sound.play()
		else:
			reload_timer.wait_time = 1.8 # Tactical reload
			reload_sound.play()
